...
class ProcessEnvironmentVarsService:
    ...
    def extract_process_from_volatility(self, file_path: str) -> list[dict]:
        raw_processes = self.i_process_vol.extract_process_environment_vars(file_path)
        return raw_processes


    def store_process_in_database(self, project_id, source_type: str, processes_lists: list[dict]) -> list[dict]:
        return self.i_process_repo.store_process_environment_vars(project_id, source_type, processes_lists)